...
//            printf("DEBUG: i2=%d\n", i);
            __m128d V_SRC_C = _mm_set_pd(SRC_C_NEXT(srcGrid), SRC_C(srcGrid));
            __m128d V_SRC_N = _mm_set_pd(SRC_N_NEXT(srcGrid), SRC_N(srcGrid));
            __m128d V_SRC_S = _mm_set_pd(SRC_S_NEXT(srcGrid), SRC_S(srcGrid));
            __m128d V_SRC_E = _mm_set_pd(SRC_E_NEXT(srcGrid), SRC_E(srcGrid));
            __m128d V_SRC_W = _mm_set_pd(SRC_W_NEXT(srcGrid), SRC_W(srcGrid));
            __m128d V_SRC_T = _mm_set_pd(SRC_T_NEXT(srcGrid), SRC_T(srcGrid));
            __m128d V_SRC_B = _mm_set_pd(SRC_B_NEXT(srcGrid), SRC_B(srcGrid));
            __m128d V_SRC_NE = _mm_set_pd(SRC_NE_NEXT(srcGrid), SRC_NE(srcGrid));
            __m128d V_SRC_NW = _mm_set_pd(SRC_NW_NEXT(srcGrid), SRC_NW(srcGrid));
            __m128d V_SRC_SE = _mm_set_pd(SRC_SE_NEXT(srcGrid), SRC_SE(srcGrid));
            __m128d V_SRC_SW = _mm_set_pd(SRC_SW_NEXT(srcGrid), SRC_SW(srcGrid));
            __m128d V_SRC_NT = _mm_set_pd(SRC_NT_NEXT(srcGrid), SRC_NT(srcGrid));
            __m128d V_SRC_NB = _mm_set_pd(SRC_NB_NEXT(srcGrid), SRC_NB(srcGrid));
            __m128d V_SRC_ST = _mm_set_pd(SRC_ST_NEXT(srcGrid), SRC_ST(srcGrid));
            __m128d V_SRC_SB = _mm_set_pd(SRC_SB_NEXT(srcGrid), SRC_SB(srcGrid));
            __m128d V_SRC_ET = _mm_set_pd(SRC_ET_NEXT(srcGrid), SRC_ET(srcGrid));
            __m128d V_SRC_EB = _mm_set_pd(SRC_EB_NEXT(srcGrid), SRC_EB(srcGrid));
            __m128d V_SRC_WT = _mm_set_pd(SRC_WT_NEXT(srcGrid), SRC_WT(srcGrid));
            __m128d V_SRC_WB = _mm_set_pd(SRC_WB_NEXT(srcGrid), SRC_WB(srcGrid));

            __m128d V_rho = +V_SRC_C + V_SRC_N
                            + V_SRC_S + V_SRC_E
                            + V_SRC_W + V_SRC_T
                            + V_SRC_B + V_SRC_NE
                            + V_SRC_NW + V_SRC_SE
            ...
            __m128d V_ux = +V_SRC_E - V_SRC_W
                           + V_SRC_NE - V_SRC_NW
                           + V_SRC_SE - V_SRC_SW
                           + V_SRC_ET + V_SRC_EB
                           - V_SRC_WT - V_SRC_WB;

            __m128d V_uy = +V_SRC_N - V_SRC_S
                           + V_SRC_NE + V_SRC_NW
                           - V_SRC_SE - V_SRC_SW
                           + V_SRC_NT + V_SRC_NB
                           - V_SRC_ST - V_SRC_SB;

            __m128d V_uz = +V_SRC_T - V_SRC_B
                           + V_SRC_NT - V_SRC_NB
                           + V_SRC_ST - V_SRC_SB
                           + V_SRC_ET - V_SRC_EB
                           + V_SRC_WT - V_SRC_WB;
            ...
            __m128d V_u2 = 1.5 * (V_ux * V_ux + V_uy * V_uy + V_uz * V_uz);

            __m128d V_feqs_C = DFL1 * V_rho * (1.0 - V_u2);
            __m128d V_feqs_N = DFL2 * V_rho * (1.0 + 4.5 * (+V_uy) * (+V_uy) - V_u2);
            __m128d V_feqs_S = V_feqs_N;
            __m128d V_feqs_E = DFL2 * V_rho * (1.0 + 4.5 * (+V_ux) * (+V_ux) - V_u2);
            __m128d V_feqs_W = V_feqs_E;
            __m128d V_feqs_T = DFL2 * V_rho * (1.0 + 4.5 * (+V_uz) * (+V_uz) - V_u2);
            __m128d V_feqs_B = V_feqs_T;
            __m128d V_feqs_NE = DFL3 * V_rho * (1.0 + 4.5 * (+V_ux + V_uy) * (+V_ux + V_uy) - V_u2);
            __m128d V_feqs_SW = V_feqs_NE;
            __m128d V_feqs_NW = DFL3 * V_rho * (1.0 + 4.5 * (-V_ux + V_uy) * (-V_ux + V_uy) - V_u2);
            __m128d V_feqs_SE = V_feqs_NW;
            __m128d V_feqs_NT = DFL3 * V_rho * (1.0 + 4.5 * (+V_uy + V_uz) * (+V_uy + V_uz) - V_u2);
            __m128d V_feqs_SB = V_feqs_NT;
            __m128d V_feqs_NB = DFL3 * V_rho * (1.0 + 4.5 * (+V_uy - V_uz) * (+V_uy - V_uz) - V_u2);
            __m128d V_feqs_ST = V_feqs_NB;
            __m128d V_feqs_ET = DFL3 * V_rho * (1.0 + 4.5 * (+V_ux + V_uz) * (+V_ux + V_uz) - V_u2);
            __m128d V_feqs_WB = V_feqs_ET;
            __m128d V_feqs_EB = DFL3 * V_rho * (1.0 + 4.5 * (+V_ux - V_uz) * (+V_ux - V_uz) - V_u2);
            __m128d V_feqs_WT = V_feqs_EB;

            __m128d V_feqa_C = _mm_set1_pd(0.0);
            __m128d
            ...
            __m128d V_feqa_S = -V_feqa_N;
            __m128d V_feqa_W = -V_feqa_E;
            __m128d V_feqa_B = -V_feqa_T;
            __m128d V_feqa_SW = -V_feqa_NE;
            __m128d V_feqa_SE = -V_feqa_NW;
            __m128d V_feqa_SB = -V_feqa_NT;
            __m128d V_feqa_ST = -V_feqa_NB;
            __m128d V_feqa_WB = -V_feqa_ET;
            __m128d V_feqa_WT = -V_feqa_EB;

            __m128d V_fs_C = V_SRC_C;
            __m128d V_fs_N = 0.5 * (V_SRC_N + V_SRC_S);
            __m128d V_fs_E = 0.5 * (V_SRC_E + V_SRC_W);
            __m128d V_fs_T = 0.5 * (V_SRC_T + V_SRC_B);
            __m128d V_fs_NE = 0.5 * (V_SRC_NE + V_SRC_SW);
            __m128d V_fs_NW = 0.5 * (V_SRC_NW + V_SRC_SE);
            __m128d V_fs_NT = 0.5 * (V_SRC_NT + V_SRC_SB);
            __m128d V_fs_NB = 0.5 * (V_SRC_NB + V_SRC_ST);
            __m128d V_fs_ET = 0.5 * (V_SRC_ET + V_SRC_WB);
            __m128d V_fs_EB = 0.5 * (V_SRC_EB + V_SRC_WT);

            __m128d V_fs_S = V_fs_N;
            __m128d V_fs_W = V_fs_E;
            __m128d V_fs_B = V_fs_T;
            __m128d V_fs_SW = V_fs_NE;
            __m128d V_fs_SE = V_fs_NW;
            __m128d V_fs_SB = V_fs_NT;
            __m128d V_fs_ST = V_fs_NB;
            __m128d V_fs_WB = V_fs_ET;
            __m128d V_fs_WT = V_fs_EB;


            __m128d V_fa_C = _mm_set1_pd(0.0);
            __m128d V_fa_N = 0.5 * (V_SRC_N - V_SRC_S);
            __m128d V_fa_E = 0.5 * (V_SRC_E - V_SRC_W);
            __m128d V_fa_T = 0.5 * (V_SRC_T - V_SRC_B);
            __m128d V_fa_NE = 0.5 * (V_SRC_NE - V_SRC_SW);
            __m128d V_fa_NW = 0.5 * (V_SRC_NW - V_SRC_SE);
            __m128d V_fa_NT = 0.5 * (V_SRC_NT - V_SRC_SB);
            __m128d V_fa_NB = 0.5 * (V_SRC_NB - V_SRC_ST);
            __m128d V_fa_ET = 0.5 * (V_SRC_ET - V_SRC_WB);
            __m128d V_fa_EB = 0.5 * (V_SRC_EB - V_SRC_WT);

            __m128d V_fa_S = -V_fa_N;
            __m128d V_fa_W = -V_fa_E;
            __m128d V_fa_B = -V_fa_T;
            __m128d V_fa_SW = -V_fa_NE;
            __m128d V_fa_SE = -V_fa_NW;
            __m128d V_fa_SB = -V_fa_NT;
            __m128d V_fa_ST = -V_fa_NB;
            __m128d V_fa_WB = -V_fa_ET;
            __m128d V_fa_WT = -V_fa_EB;

            __m128d V_DST_C = V_SRC_C - OMEGA * (V_fs_C - V_feqs_C);
            __m128d V_DST_N = V_SRC_N - OMEGA * (V_fs_N - V_feqs_N) - lambda0 * (V_fa_N - V_feqa_N);
            __m128d V_DST_S = V_SRC_S - OMEGA * (V_fs_S - V_feqs_S) - lambda0 * (V_fa_S - V_feqa_S);
            __m128d V_DST_E = V_SRC_E - OMEGA * (V_fs_E - V_feqs_E) - lambda0 * (V_fa_E - V_feqa_E);
            __m128d V_DST_W = V_SRC_W - OMEGA * (V_fs_W - V_feqs_W) - lambda0 * (V_fa_W - V_feqa_W);
            __m128d V_DST_T = V_SRC_T - OMEGA * (V_fs_T - V_feqs_T) - lambda0 * (V_fa_T - V_feqa_T);
            __m128d V_DST_B = V_SRC_B - OMEGA * (V_fs_B - V_feqs_B) - lambda0 * (V_fa_B - V_feqa_B);
            __m128d V_DST_NE = V_SRC_NE - OMEGA * (V_fs_NE - V_feqs_NE) - lambda0 * (V_fa_NE - V_feqa_NE);
            __m128d V_DST_NW = V_SRC_NW - OMEGA * (V_fs_NW - V_feqs_NW) - lambda0 * (V_fa_NW - V_feqa_NW);
            __m128d V_DST_SE = V_SRC_SE - OMEGA * (V_fs_SE - V_feqs_SE) - lambda0 * (V_fa_SE - V_feqa_SE);
            __m128d V_DST_SW = V_SRC_SW - OMEGA * (V_fs_SW - V_feqs_SW) - lambda0 * (V_fa_SW - V_feqa_SW);
            __m128d V_DST_NT = V_SRC_NT - OMEGA * (V_fs_NT - V_feqs_NT) - lambda0 * (V_fa_NT - V_feqa_NT);
            __m128d V_DST_NB = V_SRC_NB - OMEGA * (V_fs_NB - V_feqs_NB) - lambda0 * (V_fa_NB - V_feqa_NB);
            __m128d V_DST_ST = V_SRC_ST - OMEGA * (V_fs_ST - V_feqs_ST) - lambda0 * (V_fa_ST - V_feqa_ST);
            __m128d V_DST_SB = V_SRC_SB - OMEGA * (V_fs_SB - V_feqs_SB) - lambda0 * (V_fa_SB - V_feqa_SB);
            __m128d V_DST_ET = V_SRC_ET - OMEGA * (V_fs_ET - V_feqs_ET) - lambda0 * (V_fa_ET - V_feqa_ET);
            __m128d V_DST_EB = V_SRC_EB - OMEGA * (V_fs_EB - V_feqs_EB) - lambda0 * (V_fa_EB - V_feqa_EB);
            __m128d V_DST_WT = V_SRC_WT - OMEGA * (V_fs_WT - V_feqs_WT) - lambda0 * (V_fa_WT - V_feqa_WT);
            __m128d V_DST_WB = V_SRC_WB - OMEGA * (V_fs_WB - V_feqs_WB) - lambda0 * (V_fa_WB - V_feqa_WB);
        ...
        for (int count=0; count < 2; count++ ) {
            i = i + count * N_CELL_ENTRIES;
//            printf(" DEBUG: i1=%d %d %d\n", i, N_CELL_ENTRIES,  );
            if (TEST_FLAG_SWEEP(srcGrid, OBSTACLE)) {
                DST_C (dstGrid) = SRC_C (srcGrid);
                DST_S (dstGrid) = SRC_N (srcGrid);
                DST_N (dstGrid) = SRC_S (srcGrid);
