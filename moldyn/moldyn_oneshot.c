...
double *x,  *f, *vir, *epot, side, cutoffRadius;
{
  ...
  vector double xx, yy, zz, rd, rrd, rrd2, rrd3, rrd4, rrd5, rrd6, rrd7, r148;
  vector double forcex, forcey, forcez;
  vector int    i,j;
  int ii;


  sideHalf  = 0.5*side ;
  vector double side_v = {side, side};
  vector double sideHalf_v = {sideHalf, sideHalf};
  vector double negSideHalf_v = {-sideHalf, -sideHalf};

  cutoffSquare = cutoffRadius*cutoffRadius ;

  vector double cutoffSquare_v = {cutoffSquare, cutoffSquare};

  vector double vir_tmp  = {0.0, 0.0} ;
  vector double epot_tmp = {0.0, 0.0} ;

  vector double xx_tmp1;
  vector double yy_tmp1;
  vector double zz_tmp1;
  vector double xx_tmp2;
  vector double yy_tmp2;
  vector double zz_tmp2;

  vector unsigned long long xx_cmp1;
  vector unsigned long long yy_cmp1;
  vector unsigned long long zz_cmp1;
  vector unsigned long long xx_cmp2;
  vector unsigned long long yy_cmp2;
  vector unsigned long long zz_cmp2;

#pragma clang loop unroll_count(16)
  for(ii=0; ii<ninter/2*2; ii += 2) {
    i[0] = inter[INDX(ii,0)];
    i[1] = inter[INDX(ii+1,0)];

    j[0] = inter[INDX(ii,1)];
    j[1] = inter[INDX(ii+1, 1)];

    xx[0] = x[IND(0,i[0])] - x[IND(0,j[0])];
    xx[1] = x[IND(0,i[1])] - x[IND(0,j[1])];

    yy[0] = x[IND(1,i[0])] - x[IND(1,j[0])];
    yy[1] = x[IND(1,i[1])] - x[IND(1,j[1])];

    zz[0] = x[IND(2,i[0])] - x[IND(2,j[0])];
    zz[1] = x[IND(2,i[1])] - x[IND(2,j[1])];
    ...
    xx_cmp1 = vec_cmplt(xx, negSideHalf_v);
    yy_cmp1 = vec_cmplt(yy, negSideHalf_v);
    zz_cmp1 = vec_cmplt(zz, negSideHalf_v);

    xx_cmp2 = vec_cmpgt(xx, sideHalf_v);
    yy_cmp2 = vec_cmpgt(yy, sideHalf_v);
    zz_cmp2 = vec_cmpgt(zz, sideHalf_v);

    xx = vec_sel(xx, xx_tmp1, xx_cmp1);
    yy = vec_sel(yy, yy_tmp1, yy_cmp1);
    zz = vec_sel(zz, zz_tmp1, zz_cmp1);

    xx = vec_sel(xx, xx_tmp2, xx_cmp2);
    yy = vec_sel(yy, yy_tmp2, yy_cmp2);
    zz = vec_sel(zz, zz_tmp2, zz_cmp2);

//    if (xx[0] < -sideHalf) xx[0] += side;
//    if (xx[1] < -sideHalf) xx[1] += side;
//
//    if (yy[0] < -sideHalf) yy[0] += side;
//    if (yy[1] < -sideHalf) yy[1] += side;
//
//    if (zz[0] < -sideHalf) zz[0] += side;
//    if (zz[1] < -sideHalf) zz[1] += side;
//
//    if (xx[0] > sideHalf) xx[0] -= side;
//    if (xx[1] > sideHalf) xx[1] -= side;
//
...
//    if (zz[1] > sideHalf) zz[1] -= side;
    ...
    vector unsigned long long true = {1, 1};
    vector unsigned long long false = {0, 0};
    vector unsigned long long rtcheck = vec_cmplt(rd, cutoffSquare_v);
    if (vec_all_eq(rtcheck, true)) {
      rrd = 1.0/rd;
      rrd2  = rrd*rrd ;
      rrd3  = rrd2*rrd ;
      rrd4  = rrd2*rrd2 ;
      ...
      f[IND(0,i[0])]  += forcex[0] ;
      f[IND(1,i[0])]  += forcey[0] ;
      f[IND(2,i[0])]  += forcez[0] ;

      f[IND(0,i[1])]  += forcex[1] ;
      f[IND(1,i[1])]  += forcey[1] ;
      f[IND(2,i[1])]  += forcez[1] ;

      f[IND(0,j[0])]  -= forcex[0] ;
      f[IND(1,j[0])]  -= forcey[0] ;
      f[IND(2,j[0])]  -= forcez[0] ;

      f[IND(0,j[1])]  -= forcex[1] ;
      f[IND(1,j[1])]  -= forcey[1] ;
      f[IND(2,j[1])]  -= forcez[1] ;

      vir_tmp  -= rd*r148 ;
      epot_tmp += (rrd6 - rrd3);
    } else if (vec_all_eq(rtcheck, false)) {

    } else {
      if (rd[0] < cutoffSquare) {
        rrd = 1.0/rd;
        rrd2  = rrd*rrd ;
        rrd3  = rrd2*rrd ;
        rrd4  = rrd2*rrd2 ;
        ...
        f[IND(1,j[0])]  -= forcey[0] ;
        f[IND(2,j[0])]  -= forcez[0] ;
        vir_tmp[0]  -= rd[0]*r148[0] ;
        epot_tmp[0] += (rrd6[0] - rrd3[0]);
      }
      if (rd[1] < cutoffSquare) {
        rrd = 1.0/rd;
        rrd2  = rrd*rrd ;
        rrd3  = rrd2*rrd ;
        rrd4  = rrd2*rrd2 ;
        ...
        vir_tmp[1]  -= rd[1]*r148[1] ;
        epot_tmp[1] += (rrd6[1] - rrd3[1]);
      }
    }
  }
  int lane = 0;
  for(; ii<ninter; ii++) {
    int i = inter[INDX(ii,0)];
    int j = inter[INDX(ii,1)];

    double xx = x[IND(0,i)] - x[IND(0,j)];
    double yy = x[IND(1,i)] - x[IND(1,j)];
    double zz = x[IND(2,i)] - x[IND(2,j)];
    ...
    if ( rd < cutoffSquare ) {
      ...
      vir_tmp[lane]  -= rd*r148 ;
      epot_tmp[lane] += (rrd6 - rrd3);
      lane++;
    }
  }

  *vir  = vir_tmp[0] + vir_tmp[1] ;
  *epot = epot_tmp[0] + epot_tmp[1] ;
}
