//
// Created by labuser (Bangtian Liu) on 3/17/21.
//
...
#      define BOXSIZE                13
#      define NUMBER_TIMESTEPS       30
#      define MAXINTERACT       (4*BOXSIZE*BOXSIZE*BOXSIZE*4*BOXSIZE*BOXSIZE*BOXSIZE)
# endif
...
if ( rd <= cutoffSquare) {
inter[INDX(num_interact, 0)] =
i;
inter[INDX(num_interact, 1)] =
j;
num_interact ++;
...
ComputeForces_vec(
    ...
    int vec2_iters = 0;
        ...
        __m256d v_one_v1 = _mm256_set1_pd(1.0);

        __m128d v_half = _mm_set1_pd(0.5);
        __m256d v_half_v1 = _mm256_set1_pd(0.5);
        __m256i v_dim = _mm256_set1_epi32(DIMSIZE);
        __m256d v_cutoffsquare = _mm256_set1_pd(cutoffSquare);

    struct timespec begin, end;
    clock_gettime(NULL, &begin);

        for (ii = 0; ii < ninter/4*4; ii += 4) {
            v8di_t v_ij_v1, v_yy_idx_v1, v_zz_idx_v1;

            v_ij_v1.v = _mm256_loadu_si256((__m256i *) &(inter[INDX(ii, 0)]));
            v_yy_idx_v1.v = _mm256_add_epi32(v_ij_v1.v, v_dim);
            v_zz_idx_v1.v = _mm256_add_epi32(v_yy_idx_v1.v, v_dim);


            double xx0 = x[v_ij_v1.d[0]] - x[v_ij_v1.d[1]];  // i0 - j0
            double xx1 = x[v_ij_v1.d[2]] - x[v_ij_v1.d[3]]; // i1 -j1
            double xx2 = x[v_ij_v1.d[4]] - x[v_ij_v1.d[5]];
            double xx3 = x[v_ij_v1.d[6]] - x[v_ij_v1.d[7]];

            double yy0 = x[v_yy_idx_v1.d[0]] - x[v_yy_idx_v1.d[1]];//
            double yy1 = x[v_yy_idx_v1.d[2]] - x[v_yy_idx_v1.d[3]];
            double yy2 = x[v_yy_idx_v1.d[4]] - x[v_yy_idx_v1.d[5]];
            double yy3 = x[v_yy_idx_v1.d[6]] - x[v_yy_idx_v1.d[7]];

            double zz0 = x[v_zz_idx_v1.d[0]] - x[v_zz_idx_v1.d[1]];
            double zz1 = x[v_zz_idx_v1.d[2]] - x[v_zz_idx_v1.d[3]];
            double zz2 = x[v_zz_idx_v1.d[4]] - x[v_zz_idx_v1.d[5]];
            double zz3 = x[v_zz_idx_v1.d[6]] - x[v_zz_idx_v1.d[7]];
            ...
            if (xx2 < -sideHalf) xx2 += side;
            if (yy2 < -sideHalf) yy2 += side;
            if (zz2 < -sideHalf) zz2 += side;

            if (xx2 > sideHalf) xx2 -= side;
            if (yy2 > sideHalf) yy2 -= side;
            if (zz2 > sideHalf) zz2 -= side;

            if (xx3 < -sideHalf) xx3 += side;
            if (yy3 < -sideHalf) yy3 += side;
            if (zz3 < -sideHalf) zz3 += side;
            ...
            __m256d v_xx = _mm256_set_pd(xx3, xx2, xx1, xx0);
            __m256d v_xx_2 = _mm256_mul_pd(v_xx, v_xx);

            __m256d v_yy = _mm256_set_pd(yy3, yy2, yy1, yy0);
            __m256d v_yy_2 = _mm256_mul_pd(v_yy, v_yy);

            __m256d v_zz = _mm256_set_pd(zz3, zz2, zz1, zz0);
            __m256d v_zz_2 = _mm256_mul_pd(v_zz, v_zz);


            __m256d v_rd = _mm256_add_pd(_mm256_add_pd(v_xx_2, v_yy_2), v_zz_2);

            __m256d mask = _mm256_cmp_pd(v_rd, v_cutoffsquare, _CMP_LT_OS);
            int rr = _mm256_movemask_pd(mask);

//                bool condition_v1 = (v_rd[0] < cutoffSquare) & (v_rd[1] < cutoffSquare);
//                bool condition_v2 = (v_rd[2] < cutoffSquare) & (v_rd[3] < cutoffSquare);
//

            if (rr == 15) {
                __m256d v_rrd = _mm256_div_pd(v_one_v1, v_rd);
                __m256d v_rrd_2 = _mm256_mul_pd(v_rrd, v_rrd);
                __m256d v_rrd_3 = _mm256_mul_pd(v_rrd_2, v_rrd);
                __m256d v_rrd_4 = _mm256_mul_pd(v_rrd_2, v_rrd_2);
                __m256d v_rrd_6 = _mm256_mul_pd(v_rrd_2, v_rrd_4);
                __m256d v_rrd_7 = _mm256_mul_pd(v_rrd_6, v_rrd);
                __m256d v_r148 = _mm256_sub_pd(v_rrd_7, _mm256_mul_pd(v_half_v1, v_rrd_4));

                __m256d v_forcex = _mm256_mul_pd(v_xx, v_r148);
                __m256d v_forcey = _mm256_mul_pd(v_yy, v_r148);
                __m256d v_forcez = _mm256_mul_pd(v_zz, v_r148);

                f[v_ij_v1.d[0]] += v_forcex[0];
                f[v_yy_idx_v1.d[0]] += v_forcey[0];
                f[v_zz_idx_v1.d[0]] += v_forcez[0];

                f[v_ij_v1.d[2]] += v_forcex[1];
                f[v_yy_idx_v1.d[2]] += v_forcey[1];
                f[v_zz_idx_v1.d[2]] += v_forcez[1];

                f[v_ij_v1.d[4]] += v_forcex[2];
                f[v_yy_idx_v1.d[4]] += v_forcey[2];
                f[v_zz_idx_v1.d[4]] += v_forcez[2];


                f[v_ij_v1.d[6]] += v_forcex[3];
                f[v_yy_idx_v1.d[6]] += v_forcey[3];
                f[v_zz_idx_v1.d[6]] += v_forcez[3];

                f[v_ij_v1.d[1]] -= v_forcex[0];
                f[v_yy_idx_v1.d[1]] -= v_forcey[0];
                f[v_zz_idx_v1.d[1]] -= v_forcez[0];

                f[v_ij_v1.d[3]] -= v_forcex[1];
                f[v_yy_idx_v1.d[3]] -= v_forcey[1];
                f[v_zz_idx_v1.d[3]] -= v_forcez[1];

                f[v_ij_v1.d[5]] -= v_forcex[2];
                f[v_yy_idx_v1.d[5]] -= v_forcey[2];
                f[v_zz_idx_v1.d[5]] -= v_forcez[2];

                f[v_ij_v1.d[7]] -= v_forcex[3];
                f[v_yy_idx_v1.d[7]] -= v_forcey[3];
                f[v_zz_idx_v1.d[7]] -= v_forcez[3];

                __m256d v_tmp = _mm256_mul_pd(v_rd, v_r148);
                __m256d v_etmp = _mm256_sub_pd(v_rrd_6, v_rrd_3);
                vir_tmp -= (v_tmp[0] + v_tmp[1] + v_tmp[2] + v_tmp[3]);
                epot_tmp += (v_etmp[0] + v_etmp[1] + v_etmp[2] + v_etmp[3]);
            } else if (rr == 0) {
...
//                                    ++no_iters;
                        ...
                        f[v_ij_v1.d[0 + k0 * 2]] +=
                                forcex;
                        f[v_yy_idx_v1.d[0 + k0 * 2]] +=
                                forcey;
                        f[v_zz_idx_v1.d[0 + k0 * 2]] +=
                                forcez;

                        f[v_ij_v1.d[1 + k0 * 2]] -=
                                forcex;
                        f[v_yy_idx_v1.d[1 + k0 * 2]] -=
                                forcey;
                        f[v_zz_idx_v1.d[1 + k0 * 2]] -=
                                forcez;
