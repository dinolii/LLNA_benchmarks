...
//            v_zz_idx.v = _mm_add_epi32(v_yy_idx.v, v_dim);
            ...
            bool condition_xx0 = v_xx[0] < -sideHalf;
            bool condition_xx1 = v_xx[1] < -sideHalf;
            if(condition_xx0 & condition_xx1)
            {
                ++vec_iters;
                v_xx = _mm_add_pd(v_xx, v_side);
            }
            else{
                if(condition_xx0) v_xx[0] += side;
                if(condition_xx1) v_xx[1] += side;
            }

            bool bcondition_xx0 = v_xx[0] > sideHalf;
            bool bcondition_xx1 = v_xx[1] > sideHalf;

            if (bcondition_xx0 & bcondition_xx1)
            {
                ++vec_iters;
                v_xx = _mm_sub_pd(v_xx, v_side);
            }
            else {
                if(bcondition_xx0) v_xx[0] -= side;
                if(bcondition_xx1) v_xx[1] -= side;
            }


            bool condition_yy0 = v_yy[0] < -sideHalf;
            bool condition_yy1 = v_yy[1] < -sideHalf;
            if(condition_yy0 & condition_yy1)
            {
                ++vec_iters;
                v_yy = _mm_add_pd(v_yy, v_side);
            }
            else{
                if(condition_yy0) v_yy[0] += side;
                if(condition_yy1) v_yy[1] += side;
            }

            bool bcondition_yy0 = v_yy[0] > sideHalf;
            bool bcondition_yy1 = v_yy[1] > sideHalf;

            if (bcondition_yy0 & bcondition_yy1)
            {
                ++vec_iters;
                v_yy = _mm_sub_pd(v_yy, v_side);
            }
            else {
                if(bcondition_yy0) v_yy[0] -= side;
                if(bcondition_yy1) v_yy[1] -= side;
            }

            bool condition_zz0 = v_zz[0] < -sideHalf;
            bool condition_zz1 = v_zz[1] < -sideHalf;
            if(condition_zz0 & condition_zz1)
            {
                ++vec_iters;
                v_zz = _mm_add_pd(v_zz, v_side);
            }
            else{
                if(condition_zz0) v_zz[0] += side;
                if(condition_zz1) v_zz[1] += side;
            }

            bool bcondition_zz0 = v_zz[0] > sideHalf;
            bool bcondition_zz1 = v_zz[1] > sideHalf;

            if (bcondition_zz0 & bcondition_zz1)
            {
                ++vec_iters;
                v_zz = _mm_sub_pd(v_zz, v_side);
            }
            else {
                if(bcondition_zz0) v_zz[0] -= side;
                if(bcondition_zz1) v_zz[1] -= side;
            }
...
epot_tmp += (rrd6 - rrd3);
    ...
    printf("ratio=%lf\t", vec_iters*2.0/(6*ninter));
