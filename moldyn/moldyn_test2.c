...
if(v_rd[0] < cutoffSquare && v_rd[1] < cutoffSquare)
{
        ...
        if(IND(0, i0)!=IND(0, i1))
        {
            __m128d v_fx = _mm_set_pd(f[IND(0, i1)], f[IND(0, i0)]);
            __m128d v_fy = _mm_set_pd(f[IND(1, i1)], f[IND(1, i0)]);
            __m128d v_fz = _mm_set_pd(f[IND(2, i1)], f[IND(2, i0)]);

            __m128d v_rx = _mm_add_pd(v_fx, v_forcex);
            __m128d v_ry = _mm_add_pd(v_fy, v_forcey);
            __m128d v_rz = _mm_add_pd(v_fz, v_forcez);
        ...
        if(IND(0, j0)!=IND(0, j1))
        {
            __m128d v_fx = _mm_set_pd(f[IND(0, j1)], f[IND(0, j0)]);
            __m128d v_fy = _mm_set_pd(f[IND(1, j1)], f[IND(1, j0)]);
            __m128d v_fz = _mm_set_pd(f[IND(2, j1)], f[IND(2, j0)]);

            __m128d v_rx = _mm_sub_pd(v_fx, v_forcex);
            __m128d v_ry = _mm_sub_pd(v_fy, v_forcey);
            __m128d v_rz = _mm_sub_pd(v_fz, v_forcez);
