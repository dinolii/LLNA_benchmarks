...
//        __m128d v_side = _mm_set1_pd(side);
    ...
    clock_gettime(NULL, &begin);
     for(ii = 0; ii<ninter/2*2; ii+=2) {
        ...
        double xx0 = x[v_ij.d[0]] - x[v_ij.d[1]];  // i0 - j0
        double xx1 = x[v_ij.d[2]] - x[v_ij.d[3]]; // i1 -j1
        double yy0 = x[v_yy_idx.d[0]] - x[v_yy_idx.d[1]];//
        double yy1 = x[v_yy_idx.d[2]] - x[v_yy_idx.d[3]];
        double zz0 = x[v_zz_idx.d[0]] - x[v_zz_idx.d[1]];
        double zz1 = x[v_zz_idx.d[2]] - x[v_zz_idx.d[3]];
        ...
        __m128d mask = _mm_cmplt_pd(v_rd, v_cutoffsquare);
        int rr = _mm_movemask_pd(mask);
...
//                ++vec_iters;
            ...
            f[v_ij.d[0]] += v_forcex[0];
            f[v_yy_idx.d[0]] += v_forcey[0];
            f[v_zz_idx.d[0]] += v_forcez[0];

            f[v_ij.d[2]] += v_forcex[1];
            f[v_yy_idx.d[2]] += v_forcey[1];
            f[v_zz_idx.d[2]] += v_forcez[1];

            f[v_ij.d[1]] -= v_forcex[0];
            f[v_yy_idx.d[1]] -= v_forcey[0];
            f[v_zz_idx.d[1]] -= v_forcez[0];

            f[v_ij.d[3]] -= v_forcex[1];
            f[v_yy_idx.d[3]] -= v_forcey[1];
            f[v_zz_idx.d[3]] -= v_forcez[1];
